...
async def scroll_until_post_count(
    page: Page,
    target_count: int,
    *,
    max_steps: int = 40,
    ...
    """Scroll until at least target_count posts are in the DOM or steps exhausted."""
    count = await count_posts_in_dom(page)
    for _ in range(max_steps):
        if count >= target_count:
            break
        prev = count
        await scroll_feed_down(page, delay=step_delay)
        count = await count_posts_in_dom(page)
        if count <= prev:
            break
    return count
